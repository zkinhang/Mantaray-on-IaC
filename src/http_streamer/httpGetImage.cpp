...
int main() {
    ...
    std::thread server_thread([&]() {
        std::cout << "Starting MJPEG stream server on http://0.0.0.0:8080/stream" << std::endl;
        if (!svr.listen("0.0.0.0", 8080)) { // Use the original port 8080 here
    ...
    curl_easy_setopt(curl, CURLOPT_URL, "http://rov-streamer-service:5000/video_feed");
    ...
    while (true) {
        try {
            ...
            // Display the image
            cv::imshow("Video Feed", resized_img);
